...
const MappingTable = ({ data, index, status, setStatus, enableInputs, basketName, basketVal }) => {
    ...
    const handleMapping = async (customerId) => {
        console.log('clicked')
        const response = await mapBasket(basketName, adminId, customerId, broker, basketVal);
    ...
    useEffect(() => {
        ...
        if (Number(total) > Number(basketVal)) {
            console.log(total, basketVal, highlight)
            setHighlight(true);
            setEnableButtons(true);
        ...
        <tr key={index} className='border-b p-2 hover:bg-gray-100'>
            ...
            <td className=" text-sm text-black">
                <input 
                    ...
                    type='text' 
                    className={`w-28 h-8 text-right border-gray-300 bg-gray-100 rounded-md ${highlight ? 'border-red-500' : 'border-gray-300'}`}
                />
            </td>
            {/* Map status */}
            <td>
            ...
            <td className=" flex text-sm text-black mt-2 md: lg:ml-4 ">
                ...
                <Tooltip content="Map Customer">
                    <button disabled={enableButtons} className="ml-2" onClick={() => {handleMapping(data.customerId)}}>
                ...
                <Tooltip content="Send Weblink">
                    <button disabled={enableButtons} className="ml-4">
                        <svg className="w-4 h-4 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 18">
